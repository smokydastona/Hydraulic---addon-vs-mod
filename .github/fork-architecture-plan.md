...
- Bedrock block-use inventory transactions now route through a non-cancelling Geyser translator mixin into the runtime action router. The router creates a `RuntimeTraceId`, extracts the concrete block position from `InventoryTransactionPacket` `ITEM_USE` action type `0`, resolves the current Java player level, and performs traced runtime target discovery through the Geyser-session source. It records explicit `TARGET_RESOLVED`, `TARGET_UNAVAILABLE`, `CAPABILITY_UNAVAILABLE`, or `IGNORED` outcomes without inventing a generic mutation from a click. Focused tests verify block-use routing, ignored non-block transactions, missing position, missing target, and missing capability behavior.
...
2. translate selected Bedrock interaction/menu actions into authoritative transfer or property mutations only when the compiled plan provides concrete slot, tank, energy, or property semantics
