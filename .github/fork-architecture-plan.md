...
1. run a live Fabric/Geyser/Bedrock synchronization regression that proves transaction-generated inventory slot updates are observed by a Bedrock client, and expand transport mappings for non-inventory state only where a concrete Bedrock packet/state target exists
2. compile mixed-resource machine recipe facts from compatibility plans instead of requiring callers to provide recipe objects manually
