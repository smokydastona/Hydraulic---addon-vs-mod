...
4. move model loading to lazy indexed access, then widen that approach to the remaining resource categories; startup lookup and custom model conversion now use indexed model paths, the first shared texture-resolution cache and first selective texture-conversion dependency graph slices are now shipped, but broader indexed texture and resource loading is still eager
