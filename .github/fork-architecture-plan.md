...
- Live Bedrock-client synchronization verification, Bedrock action routing into target-discovery transfer execution, richer menu behavior, block-entity behavior, deeper entity behavior, custom networking, custom rendering analysis, and broad automatic machine recipe discovery remain incomplete.
...
2. route Bedrock interaction/menu actions into `RuntimeTargetDiscovery.forGeyserSession(...)` so real client actions can resolve block positions, capabilities, and transfer requests without caller-supplied target objects
