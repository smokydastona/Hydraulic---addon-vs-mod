...
- Runtime diagnostics now consume typed bridge-group queries for menu and block-entity candidate selection, fluid presentation has a first real bucket-item consumer, generic machine processing has an executable item-transfer-backed bridge, inventory and sided automation access are executable through direct dispatch, normalized container-to-tank transfer mutates real fluid state with identity enforcement, and mixed item/fluid/energy machine operations can now be represented atomically. Broader Java recipe discovery, fluid block/state translation, mod-specific machine semantics, and real Bedrock sync transport remain outside those generic contracts.
...
1. evolve machine execution from item-only `MachineProcessingBridge` completion into a compiled mixed-resource executor that consumes `MultiResourceTransaction`
...
3. wire automation capability execution onto the shared transaction/request path so sided item, fluid, and energy movement no longer depends on caller-specific resource logic
