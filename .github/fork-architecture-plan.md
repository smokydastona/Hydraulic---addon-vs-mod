...
- Runtime target discovery now has a shared resolver that maps a world position key to a runtime target, resolves executable item/fluid/energy automation capabilities through the compiled runtime dispatch table, builds typed transfer requests, and executes them through the transaction and dirty-state path. Focused tests verify position-based item, fluid, and energy transfers, synchronization after a discovered item transfer, missing-target rejection, and missing-capability rejection. Live platform wiring from actual block-entity/world capability lookup into this resolver remains a Fabric/Geyser integration gate.
...
- Live Bedrock-client synchronization verification, platform-backed world target discovery, richer menu behavior, block-entity behavior, deeper entity behavior, custom networking, custom rendering analysis, and broad automatic machine recipe discovery remain incomplete.
...
2. wire platform-backed world/block-entity capability lookup into `RuntimeTargetDiscovery` so real block positions resolve item, fluid, and energy handlers without caller-supplied target objects
