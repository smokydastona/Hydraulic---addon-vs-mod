...
- Synchronization planning and encoding now consume transaction-generated dirty state into Bedrock-facing encoded records, but synchronization transport remains incomplete until those encoded records are handed to a real Hydraulic/Geyser session and verified as delivered or rejected at that boundary.
...
- Real synchronization transport, richer menu behavior, block-entity behavior, deeper entity behavior, custom networking, custom rendering analysis, and broad machine recipe discovery remain incomplete.
...
1. implement the synchronization transport boundary from `EncodedSyncChange` batches into a real Hydraulic/Geyser session handoff with explicit queued, sent, failed, and target-unavailable results
2. evolve machine execution from item-only `MachineProcessingBridge` completion into a compiled mixed-resource executor that consumes `MultiResourceTransaction`
