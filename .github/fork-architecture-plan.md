...
- Runtime diagnostics now consume typed bridge-group queries for menu and block-entity candidate selection, fluid presentation has a first real bucket-item consumer, generic machine processing has an executable item-transfer-backed bridge, and normalized container-to-tank transfer now mutates real fluid state. Broader Java recipe discovery, fluid block/state translation, and mod-specific machine semantics remain outside those generic contracts.
