...
- Runtime diagnostics now consume typed bridge-group queries for menu and block-entity candidate selection, fluid presentation has a first real bucket-item consumer, and generic machine processing now has an executable item-transfer-backed bridge. Broader Java recipe discovery, fluid execution, and mod-specific machine semantics remain outside that generic contract.
