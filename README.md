...
That menu fallback path now also compiles the validated fallback container directly into the runtime plan as a typed `ContainerType`, so the unsupported-open-screen bridge no longer reparses container enum names from strings when it fires.
